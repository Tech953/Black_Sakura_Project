...
- Mobile: AsyncStorage is async — all reads go through the exported `i18nReady` hydration promise and `resolveReplyLanguage()` is async (await it at send time); selection-version guards stop a slow hydration read from clobbering a newer in-session choice. **Known limitation: mobile has no RTL layout handling for Arabic (text translates, layout stays LTR; I18nManager would need an app restart).**
