...
**Seeding:** the archive seeds atomically (one transaction: engram + conversation + exactly 1,534 ordered messages) via shared pure canonical data. Server/desktop use `seedAll`; mobile SQLite lazily evaluates the large transcript only on first missing-archive initialization, marks canonical rows already synced, and reuses the preserved conversation for history. A repeated server seed never repairs or mutates: it validates the canonical engram, sole conversation, exact role/content sequence, and deterministic timestamps, then fails loudly on drift. Dev/Replit Postgres has no auto-seed — run the scripts package's `seed:full-rezz`.
